...
  const wheel = track.closest(".projects-wheel");
...
  function disableInteractiveElements(element) {

    element.querySelectorAll("a, button, input, select, textarea, [tabindex]").forEach((node) => {
...
    const directCards = Array.from(track.querySelectorAll(":scope > .project-shell"));

    // Save the original source cards once

    if (!track.dataset.sourceHtml) {

      if (!directCards.length) return;

      track.dataset.sourceHtml = directCards.map((card) => card.outerHTML).join("");

    }

    // Reset and rebuild from original source

    track.innerHTML = "";

    const originalGroup = document.createElement("div");

    originalGroup.className = "projects-group is-original";

    originalGroup.innerHTML = track.dataset.sourceHtml;

    track.appendChild(originalGroup);

    // Measure one full group

    const groupWidth = Math.ceil(originalGroup.getBoundingClientRect().width);

    const wheelWidth = wheel.clientWidth;

    // Create enough extra groups so the track never runs out while scrolling

    const extraGroupsNeeded = Math.max(

      1,

      Math.ceil((wheelWidth + groupWidth) / groupWidth)

    );

    for (let i = 0; i < extraGroupsNeeded; i += 1) {
...
      disableInteractiveElements(cloneGroup);

      track.appendChild(cloneGroup);

    }

    // Set animation distance and duration

    const SPEED_PX_PER_SEC = 90; // change this if you want faster/slower

    const duration = groupWidth / SPEED_PX_PER_SEC;
...
    prefersReducedMotion.addEventListener("change", buildMarquee);
